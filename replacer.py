...
LastEditTime: 2021-02-04 08:40:12
'''
...
>>> import replacer
>>> new_text = replacer.spacing_text('在中文English之间插入空格')
>>> print(new_text)
'在中文 English 之间插入空格'
>>> new_content = replacer.spacing_file('path/to/file.txt')
>>> print(new_content)
'一款在中文和 English 之间插入空格的插件'
"""
...
CONVERT_TO_FULLWIDTH_CJK_SYMBOLS = re.compile(
    '([{CJK}])[ ]*([~\\!;,\\?]+)[ ]*'.format(CJK=CJK))
DOTS_CJK = re.compile('([\\.]{{2,}}|\u2026)([{CJK}])'.format(
    CJK=CJK))  # need to escape { }
FIX_CJK_COLON_ANS = re.compile('([{CJK}])\\:([A-Z0-9\\(\\)])'.format(CJK=CJK))

CJK_QUOTE = re.compile('([{CJK}])([`"\u05f4])'.format(
    CJK=CJK))  # no need to escape `
QUOTE_CJK = re.compile('([`"\u05f4])([{CJK}])'.format(
    CJK=CJK))  # no need to escape `
...
RIGHT_BRACKET_CJK = re.compile('([\\)\\]\\}}<>\u201d])([{CJK}])'.format(
    CJK=CJK))  # need to escape }
...
CJK_ANS = re.compile(
    '([{CJK}])([A-Za-z\u0370-\u03ff0-9@\\$%\\^&\\*\\-\\+\\\\=\\|/\u00a1-\u00ff\u2150-\u218f\u2700—\u27bf])'.format(CJK=CJK))
ANS_CJK = re.compile(
    '([A-Za-z\u0370-\u03ff0-9~\\!\\$%\\^&\\*\\-\\+\\\\=\\|;:,\\./\\?\u00a1-\u00ff\u2150-\u218f\u2700—\u27bf])([{CJK}])'.format(CJK=CJK))
...
def spacing(text):
    ...
    while matched:
        start, end = matched.span()
        new_text = ''.join((new_text[:start + 1].strip(), convert_to_fullwidth(
            new_text[start + 1:end]), new_text[end:].strip()))
        matched = CONVERT_TO_FULLWIDTH_CJK_SYMBOLS.search(new_text)
...
def cli(args=None):
    ...
    parser = argparse.ArgumentParser(
        prog='replacer',
        description='replacer.py -- Paranoid text spacing for good readability, to automatically insert ' +
        'whitespace between CJK and half-width characters (alphabetical letters, numerical digits and symbols).',
    )
    parser.add_argument('-t', '--text', action='store_true', dest='is_text',
                        required=False, help='specify the input value is a text')
    parser.add_argument('-f', '--file', action='store_true', dest='is_file',
                        required=False, help='specify the input value is a file path')
    parser.add_argument('text_or_path', action='store',
                        type=str, help='the text or file path to apply spacing')
